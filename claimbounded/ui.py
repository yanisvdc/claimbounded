...
def _generate_profile_docx(profiles: list, title: str = "Device Profile Report") -> str | None:
    ...
    h1.add_run(title)
    p = doc.add_paragraph()
    _run(p, f"{len(profiles)} device(s) · {today} · Grounded in 1,400 public FDA authorization records",
    ...
    for i, profile in enumerate(profiles, 1):
        ...
        for label, key in [
            ...
            ("Postmarket evaluability class", "postmarket_evaluability_class"),
            ("Authorization endpoint recoverability", "authorization_endpoint_recoverability"),
...
def _generate_html_report(pkg: dict[str, Any]) -> str:
    ...
    meta_parts.append("Grounded in 1,400 public FDA authorization records")
...
def _generate_docx_report(pkg: dict[str, Any]) -> str | None:
    ...
    _run(p2, "Grounded in 1,400 public FDA 510(k) and De Novo authorization records", italic=True, color="#64748b", size=10)
    doc.add_paragraph()
  ...
  <div style="background:#6b21a8;border-radius:12px;padding:20px">
    ...
    with gr.Blocks(title="claimbounded") as demo:
        ...
        gr.Markdown(
            ...
            "grounded in **1,400 public FDA authorization records** (510(k) and De Novo summaries)."
        )
        ...
        with gr.Tabs():
            ...
            with gr.Tab("② Corpus Search"):
                gr.Markdown(
                    "Search the **1,400-device FDA corpus** by device name, manufacturer, or intended use. "
